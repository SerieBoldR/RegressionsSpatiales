...
-   Pour calculer des vecteurs propres de Moran :
    -   `adespatial` dédié l'analyse spatiale multidimensionnelle.
...
## Les vecteurs propres de Moran {#sec-071}
...
Il existe actuellement trois approches permettant de sélectionner les vecteurs propres spatiaux à intégrer dans un modèle. Ces trois approches partent de postulats différents et ont chacune leurs avantages et inconvénients que nous décrirons dans les trois prochaines sous-sections. 

### Sélection itérative {#sec-0721}
...
## Mise en œuvre et analyse dans R {#sec-073}

Pour illustrer l'utilisation des modèles SEVM, nous utilisons le même exemple que pour les modèles GAM ([section @sec-065]). Nous avons mentionné un peu plus haut que ces deux types de modèles se ressemblaient beaucoup dans leur façon d'intégrer l'espace dans leur formulation. 
...
### Modèle SEVM par sélection itérative {#sec-0731}

La première approche pour développer un SEVM consiste à sélectionner les vecteurs propres spatiaux à l'aide de la méthode décrite à la [section @sec-0721]. L'approche est notamment implémentée dans le *package* `spatialreg`.
...
#| warning: false
#| label: fig-ressev2
#| fig-cap: Résidus simulé du modèle SEVM avec distribution de Student par sélection pas à pas
#| fig-align: center
#| out-width: 85% 
...
Nous notons une nette amélioration dans la distribution des résidus (@fig-ressev2). Nous effectuerons l'interprétation des résultats de ce modèle en comparaison avec ceux obtenus par les deux autres versions ([section @sec-0734]).

### Modèle SEVM avec effet aléatoire {#sec-0732}
...
Cependant, comme nous l'avons mentionné plus haut ([section @sec-0722]), la pénalisation des coefficients d'un modèle induite par leur introduction comme un effet aléatoire est équivalente à leur pénalisation par la méthode *ridge* (*l1 regularization*). Il existe plusieurs *packages* dans R permettant d'implémenter ce type de pénalisation dans des GLM dont `glmnet`, `ridge`, et `gamlss`. Le package `mgcv` que nous avons présenté dans la section sur les modèles GAM est également capable d'inclure une pénalité de type *ridge* sur les coefficients des termes linéaires. Il faut pour cela utiliser un paramètre méconnu appelé `paraPen`. Considérant que la distribution de Student a donné des bons résultats jusqu'ici avec la fonction `gam` et le paramètre `family = scat`, nous allons continuer à l'utiliser. De plus, le package `mgcv` est capable de déterminer automatiquement le paramètre $\lambda$ calibrant la force de la pénalisation.
...
### Modèle SEVM avec pénalisation Lasso {#sec-0733}
...
### Comparaison des trois modèles SEVM {#sec-0734}
...
Les effets spatiaux capturés par les trois modèles sont très similaires (@fig-sevm6). La corrélation est cependant beaucoup plus élevée entre le terme spatial du modèle RE-SEVM et LASSO-SEVM (`r format(round(cor(pred_ridge, pred_lasso),2), decimal.mark =",")`) qu'avec le modèle SEL-SEVM (`r format(round(cor(pred_ridge, pred_sel),2), decimal.mark =",")`). Une différence notable s'observe pour l'arrondissement du Plateau-Mont-Royal : les modèles RE-SEVM et LASSO-SEVM ont des termes spatiaux avec des valeurs bien plus élevées pour ce secteur dans lequel la part modale active (marche + vélo) est très élevée. Ceci explique probablement la réduction de l'effet de la variance indépendante d'accessibilité à pied aux emplois dans ces modèles.
