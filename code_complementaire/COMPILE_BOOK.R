...
if(author == "PA"){
  ...
  username <- "JeremyGelb"
  book_dir <- 'E:/Livres/RegressionsSpatiales'
}
...
paths <- c(
  list.files('data', pattern = '*.rda', full.names = TRUE, recursive = TRUE),
  list.files('data', pattern = '*.RData', full.names = TRUE, recursive = TRUE))
