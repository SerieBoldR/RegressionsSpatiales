...
3.  La prise en compte de l'autocorrélation spatiale est paramétrique, car elle suit une fonction prédéterminée.
...
Le modèle MCO explique près de 45 % de la variance du prix des logements.
...
#| fig-cap: Fonction de corrélation estimée par le modèle GLS
#| fig-align: center
#| out-width: 75%
...
Notre modèle GLS a donc pu retirer l'autocorrélation qui restait dans les résidus du modèle MCO initial. Nous pouvons donc à présent vérifier la distribution de ses résidus.
...
-   Pour le modèle MCO, le fait de disposer d'un stationnement gratuit avec le logement a un impact non significatif. Pour le modèle GLS, il contribue à augmenter le prix par nuit de `r format(round(exp(df2$Value[[4]]) * 100 - 100,2), decimal.mark = ",")`% (p = `r format(round(df2$p.value[[4]],3), decimal.mark = ",")`).
...
Ce modèle formule donc une hypothèse de dépendance conditionnelle entre les résidus des observations voisines. Le modèle CAR s'inspire directement des chaînes et des champs de Markov dont il tire une propriété centrale : la valeur de l'effet spatial d'une observation *i* ne dépend que de celles de ses observations voisines *j*. Pour comprendre l'impact du paramètre $\rho$, nous pouvons visualiser à la @fig-carexamples différentes réalisations d'une distribution normale centrée sur zéro avec une structure de variance-covariance suivant un modèle CAR.
...
#| fig-cap: Exemples de réalisations d'une variable avec une distribution normale avec un modèle CAR
#| fig-align: center
#| out-width: '100%'
...
kbl(tab_coeff,
    ...
    row.names = FALSE,
  	format.args = list(decimal.mark = ',', big.mark = " "),
  	col.names = c('Variable',"Coefficient","exp(coefficient)","Valeur de p"),
  	align= c('l',"r","r","r"),
...
Nous commençons par ajuster un modèle non spatial.
...
#### Modèle GLMM avec un effet aléatoire de type CAR {#sec-02321}
...
À la lecture du @tbl-ResultatsModelCAR, plusieurs constats peuvent être avancés :
...
#### Modèle GLMM avec un effet aléatoire avec la fonction Matérn {#sec-02322}
...
#| fig-align: center
#| layout-ncol: 2
#| fig-cap: "Structures de corrélation ajustées par le modèle"
#| fig-subcap: 
#|   - "Structure Cauchy"
...
Les deux cartes sont extrêmement similaires avec uniquement des changements mineurs (@fig-glmmRESp). 


#### Compararaison des trois modèles GLMM avec des effets aléatoires spatiaux {#sec-02323}

Pour terminer, nous pouvons comparer la qualité d'ajustement des trois modèles GLMM avec les AIC (@tbl-TroisModelesAIC).

```{r}
#| label: tbl-TroisModelesAIC
#| tbl-cap: Valeurs des AIC marginaux et conditionnels pour les trois modèles
#| #| echo: true 
#| message: false 
#| eval: true
#| warning: false
