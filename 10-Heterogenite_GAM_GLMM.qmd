...
Maintenant que nous disposons de notre *mesh* et que nous avons appliqué les transformations aux données, nous ajustons un modèle de référence dans lequel aucun coefficient ne varie spatialement. Nous avons seulement une constante spatiale, comme réalisé dans la [section @sec-0232].
...
#| fig-cap: 'Résidus simulés du modèle sans coefficient variant spatialement'
...
Le @tbl-Model2EffetsSpatiaux indique clairement que trois de nos sept coefficients spatiaux n'ont pas de variation spatiale significative (`IC 2,5 % < 0.01`). Nous allons donc réajuster le modèle en retirant ces trois termes (`cr_dist_to_airport, cr_per_hispanic, cr_per_black`).
...
La nouvelle version du modèle incluant nos coefficients variant significativement spatialement obtient également de bons résidus simulés (@fig-heteroresid2).
...
L'AIC conditionnel du second modèle est plus petit que celui du premier modèle, indiquant un meilleur ajustement. À titre de rappel, l'AIC conditionnel est une version du AIC plus adaptée pour comparer des modèles à effets mixtes si leurs effets aléatoires sont différents (voir la [section @sec-0232]). 
...
Attention, dans les cartes ci-dessus (figures [-@fig-glmmheterospacecoeff-1] à [-@fig-glmmheterospacecoeff-5]), les comtés grisés correspondent à des comtés pour lesquels le coefficient cartographié n'a pas significativement varié spatialement. Dans ces cas-ci, l'effet qui s'applique dans le comté est tout simplement l'effet fixe général pour ce coefficient.
