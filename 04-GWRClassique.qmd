...
tab    <- summary(modele_global)
tab_reg <- as.data.frame(tab$coefficients)
tab_reg$Variable <- rownames(tab_reg)
tab_reg <- tab_reg[, c(5, 1:4)]
tab_reg[1, 1] <- "Constante"
...
r2_global <- as.character(round(r2_global,4))
r2_global <- gsub("\\.", ",", r2_global) 

rss <- sum((modele_gwr$lm$y - modele_gwr$SDF$pred)^2) 
tss <- sum((modele_gwr$lm$y - mean(modele_gwr$SDF$pred))^2) 
r2_gwr <- 1 - (rss / tss)
r2_gwr <- as.character(round(r2_gwr,4))
r2_gwr <- gsub("\\.", ",", r2_gwr) 

note <- paste0("R carré du modèle global = ", r2_global, ". R carré du modèle GWR = ", r2_gwr, ".") 
## Anova
gwr_anova <- anova(modele_gwr)
tableau_anova <- data.frame(Modèle = c("Résidus du modèle global",
                                      "Modèle GWR",
                                      "Résidus du modèle GWR"),
                            dl = gwr_anova$Df,
                            SumSq = gwr_anova$`Sum Sq`,
                            MeanSq = gwr_anova$`Mean Sq`,
                            Fisher = gwr_anova$`F value`)
...
LyonIris$plus_sign <-  factor(plus_sign, 
...
carte2 <- tm_shape(gwr_resultats)+ tm_borders(col="gray25", lwd=.5)+
