...
### Jeu de données sur les parts modales de la région métropolitaine de Montréal {#sec-0112}
...
intitule <- c("Proportion de familles monoparentales",
              ...
              "Accessibilité aux emplois en heure de pointe en transport en commun",
...
#| fig-cap: Parts modales du transport en commun, de l'automobile et du transport actif (proportion)
#| fig-align: center
...
donnees <- st_drop_geometry(Barcelone)
donnees <-  donnees[c("NO2", "PM25", "Arbres","Surf", "Parcs",
                      "PopTot", "Densite", "Revenu", "Pct_Img", "Pct0_4", "Pct_Plus80", 
                      "Distance", "ClusterUE")]
 
intitule <- c("Dioxyde d'azote (ug/m^3^)",
              "Particules fines (PM$_{2,5}$)",
              "Nombre d'arbres",
              "Surface bâtie (m^2^)",
              "Espaces verts (m^2^)",
              "Population",
              "Habitants au km^2^",
              "Revenu des ménages (Euros)",
              "Immigrants non européens (%)",
              "Moins de 4 ans (%)",
              "80 ans et plus (%)",
              "Distance du centre (m)",
              "Cluster of UE Immigrants")

stats <- data.frame(variable = names(donnees),
                    nom = intitule,
                    type = c("EN", "EN", "EN", "EN", "EN", 
                             "SE", "SE", "SE", "SE", "SE", "SE", 
                             "GE", "BI"),
                    moy = round(sapply(donnees, mean),2),
                    et = round(sapply(donnees, sd),2),                    
...
![Cartographie des variables du jeu de données de Barcelone](images/Chap01/FigureDataBarcelone.png){#fig-barceloneCarto width="100%" fig-align="center"}

### Jeu de données sur les taux de décès dde la COVID19 dans les comtés des États-Unis {#sec-0114}

Quatrièmement, nous utiliserons un jeu mis l'étude de @kaashoek2022evolving. Dans cet article, les auteurs cherchent à déterminer quels sont les facteurs socio-économiques, politiques, et sanitaires qui ont été associés avec une prévalence de décès de COVID19 dans les différents comtés des États-Unis. Les données de l'article ont été publiées en [accès ouvert](https://github.com/ctesta01/spatial_poisson_covid/tree/main). Nous nous concentrons spécifiquement sur les données utilisées pour réaliser modèle spatial appliqué à la troisième période d'étude.

L'objectif de ce modèle est donc de prédire le nombre de décès de COVID19 par comté pour la période octobre 2020 à février 2021. Il s'agit de la période pendant laquelle le nombre de cas était le plus élevé pour l'ensemble du pays. La première période correspond aux premiers pics épidémiques (férvier - mai 2020) et à l'apparition des premiers clusters de contagions. La seconde période a été marquée par l'adoption des premières mesures sanitaires et une poursuite de la contamination vers les états plus éloignés et moins peuplés. 
...
### Jeu de données sur les prix des Airbnb de l'Île de Montréal {#sec-0115}

Ce jeu de donnée comporte 2500 logements airbnb pour lesquels nous connaissons un ensemble d'informations sur la nature du logement, son propriétaire ainsi que l'environnement à proximité du logement. Les données sont mises à disposition par l'organisation [Inside Airbnb](https://insideairbnb.com/fr/get-the-data/). La version utilisée ici a été téléchargée en 2023 et est échantillon aléatoire de tous les logements disponibles.

Nous avons notamment accès aux variables suivantes : 

* `price` : le prix en dollars du logement pour une nuit
* `private` : une variable binaire indiquant si le logement est privé ou partagé
* `stationnement_gratuit` : une variable binaire indiquant si le logement dispose d'un stationnement gratuit
* `jardin_ou_cours` : une variable binaire indiquant si le logement dispose d'un jardin ou d'une cours
* `bedrooms` : une variable de comptage indiquant le nombre de chambres dans le logement
* `super_hote` : une variable binaire indiquant si l'hôte à le statut *super-hôte*
* `number_of_reviews` : une variable de comptage indiquant le nombre d'évaluations reçues par le logement
* `review_scores_rating` : une variable continue indiquant la note sur 100 du logement
* `metro_500m` : une variable binaire indiquant si le logement est à moins de 500m d'une station de métro
* `prt_veg_500m` : une variable continue indiquant la part de la superficie du logement dans un rayon de 500 couverte par de la végétation haute (arbres).
...
## Matrices de pondération spatiale {#sec-012}
...
Habituellement appelée $\mathbf{W}$, la matrice de contiguïté est binaire tant selon le partage d'un nœud (*Queen* en anglais) (@eq-ContiguiteQueen) que d'un segment commun (*Rook* en anglais) (@eq-ContiguiteRook).
...
Pour construire une matrice de pondération spatiale selon la proximité, nous pouvons utiliser plusieurs types de distances [@ApparicioGelbrevisited] : certaines sont cartésiennes, d'autres, dites réticulaires, sont calculées à partir d'un réseau de rues (@fig-Chap01TypesDistances).
...
::: bloc_astuce-body
À la [section @sec-0122], nous avons vu que plusieurs types de distance peuvent être utilisés : cartésiennes (euclidienne et de Manhattan) et réticulaires (chemin le plus rapide à pied, à vélo, en automobile et en transport en commun).
...
Pour intégrer une distance réticulaire, vous devez la calculer, soit [dans R](https://serieboldr.github.io/MethodesAnalyseSpatiale/01-ManipulationDonneesSpatiales.html) [@RBoldAirMethodesAnalysesSpatiales], soit dans un logiciel de système d'information géographique (QGIS ou ArcGIS Pro avec l'extension *Network Analyst* par exemple) et l'importer dans R. Le reste du code sera alors identique.
:::
:::

Nous avons vu qu'il est possible d'utiliser une matrice en fixant une distance maximale au-delà de laquelle les pondérations sont mises à 0 (@eq-MatriceDistance2). Le code ci-dessous permet de créer des matrices de pondération standardisée avec l'inverse de la distance et l'inverse de la distance au carré avec des seuils de 2500 et de 5000 mètres.
...
#| fig-align: center
#| fig-cap: Exemple de variable spatialement décalée (dioxyde d'azote)
#| out-width: 85%
# Chargement des données
library(tmap)
library(spdep)
...
moran_resultats$MoranIs <- round(moran_resultats$MoranIs,4)
moran_resultats$Descrip <- c("Partage d’un nœud", "Partage d’un segment", "Ordre 2", "Ordre 3", "Ordre 4", "Ordre 5",
                             "2500 mètres",
...
moran_resultats <- moran_resultats[, c(1,4,2,3)]
my_table <- knitr::kable(moran_resultats,
           format.args = list(decimal.mark = ',', big.mark = " "),
...
#| fig-align: center 
#| fig-cap: Valeurs du *I* de Moran pour les dix variables 
#| out-width: 75%
...
::: bloc_aller_loin-body
Afin de bien maîtriser les principes de base et les hypothèses de la régression linéaire multiple, de mesurer la qualité d'ajustement du modèle, d'introduire des variables explicatives particulières (variable qualitative dichotomique ou polytomique, variable d'interaction, etc.), d'interpréter les résultats d'un modèle de régression et de le mettre en œuvre dans R, nous vous invitons à lire le chapitre intitulé [Régression linéaire multiple](https://serieboldr.github.io/MethodesQuantitatives/07-regressionlineaire.html) [@RBoldAirMethodesQuanti].
:::
:::

## Pourquoi recourir à des régressions spatiales? {#sec-016}

Deux motivations principales peuvent conduire à réaliser des régressions spatiales : la **dépendance spatiale** ou l'**hétérogénéité** du modèle de régression classique appliqué à des données spatiales [@chi2019spatial, p. 41-43].
...
Le **modèle d'autorégression conditionnelle** (CAR) permet de tenir compte de la dépendance locale de la variable dépendante ([chapitre @sec-chap02]).

**Les modèles d'économétrie spatiale** ([chapitre @sec-chap03], [chapitre @sec-chap04] et [chapitre @sec-chap05]) sont des solutions particulièrement intéressantes pour remédier au problème de dépendance spatiale d'un modèle et obtenir des coefficients de régression non biaisés, et ce, en introduisant l'autocorrélation spatiale de différentes façons : sur les variables indépendantes, sur la variable dépendante, sur le terme d'erreur, etc.).

**Les modèles linéaires généralisés** (GAM) ([chapitre @sec-chap06]) permettent d’introduire des lissages spatiaux pour capturer les tendances spatiales soit de manière continue (avec une spline bivariée sur les coordonnées géographiques), soit de manière discrète (avec un lissage par champ aléatoire de Markov). Aussi, les **modèles linéaires généralisés avec des vecteurs spatiaux** ([chapitre @sec-chap07]) permettent de capturer les tendances spatiales en résumant la matrice de pondération spatiale $W$.
:::
:::

### Hétérogénéité spatiale {#sec-0172}
...
:::
::: bloc_aller_loin-body
**Les régressions géographiquement pondérées** ([chapitre @sec-chap08] et [chapitre @sec-chap09]), sont des outils d'exploration particulièrement adaptés pour cartographier et analyser l'instabilité des relations entre la variable dépendante et les variables indépendantes d'un modèle de régression.

Les **modèles généralisés additifs** (GAM) et les **modèles généralisés à effet mixtes** (GLMM) permmettent aussi d'intégrer des coefficients variant spatialement ([chapitre @sec-chap09])
:::
:::

## Quiz de révision {#sec-017}
...
à compléter
```
Correction à la [section @sec-11011].
:::
:::
...
à compléter
```
Correction à la [section @sec-11012].
:::
:::
