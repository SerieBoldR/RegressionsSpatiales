...
## Bref retour sur les GAM {#sec-061}
...
Ces fonctions *f* sont le plus souvent estimées avec des *splines*. Le fonctionnement de base est assez simple. Pour chaque variable *X* devant suivre une relation non linéaire, la variable est remplacée par *k* sous-variable (*x1*, *x2*, ..., *xk*). *k* représente le niveau de complexité attendu de la relation, plus *k* est grand, plus la fonction à ajuster pourra être complexe. Le modèle va estimer un coefficient pour chacune de ces sous-variables, et la somme des effets de ces sous-variables permet de reconstruire la relation non inéaire.

La @fig-gam1 permet d'illustrer ce principe. Les lignes grises représentent les sous-variables, aussi appelées bases, qui sont ensuite multiplées par leur coefficient respectif pour obtenir leurs effets (en couleur), puis la ligne bleue représente leur somme. La ligne bleue capture très efficacement la relation entre la variable X et Y.
...
## Comment utiliser une spline pour ajouter l'espace dans un modèle GAM? {#sec-062}
...
## Pourquoi recourir à un modèle GAM? {#sec-063}
...
## *Random Markow Field* (RFM) ou spline bivariée? {#sec-064}
...
## Mise en oeuvre et analyse dans R {#sec-065}

Pour illustrer la réalisation des différentes modèles GAM (non spatiale, avec une spline bivariée sur les coordonnées géographiques et avec une spline spatiale de type RMF), nous utilisons le jeu de données sur les parts modales dans la région métropolitaine de Montréal ([section @sec-0112]). Ce jeu de données comprend trois variables mesurant respectivement la proportion des individus utilisant la voiture, le transport collectif ou un mode actif (marche ou vélo) comme mode de transport principal pour ses déplacements domicile-travail en 2021 pour les secteurs de recensement (@fig-datamtlCarto).
...
L'analyse de données compositionnelles est un domaine en soit et sort du matériel que nous souhaitons couvrir dans ce livre. Il existe d'excellentes ressources et packages R sur le sujet [@van2013analyzing; @comp_pkg2024; @comp_pkg2025].
...
### Modèles GAM sans intégration de l'espace {#sec-0651}
...
::: bloc_aller_loin-body
Pour une description des résidus simulés, vous pourrez consulter [ce chapitre sur les GLM du livre](https://serieboldr.github.io/MethodesQuantitatives/09-GLMM.html) du livre *Méthodes quantitatives en sciences sociales : un grand bol d'R* [@RBoldAirMethodesQuanti].
:::
:::
...
#| warning: false
#| label: fig-gamresidA
#| fig-cap: Résidus du simple GLM avec une distribution noramle
#| fig-align: center
#| out-width: 85%
library(DHARMa)
library(mgcViz)
...
### GAM avec spline spatiale bivariée sur les coordonnées géographiques {#sec-0652}
...
La valeur du I de Moran (I = `r format(round(as.numeric(test$statistic),3), decimal.mark =",")`), calculé à partir de la matrice de contiguïté selon le partage d'un nœud (*Queen*), indique toujours une autocorrélation spatiale non négligeable des résidus (@fig-gamxycarto).
...
Nous constatons alors que le modèle est parvenu à retirer l'autocorrélation spatiale des résidus (I de Moran de 0,026), si elle est évalué avec la façon dont l'espace est prise en compte dans le modèle. Interprétons maintenant les résultats du modèle avec la fonction `summary`.
...
La @fig-gamspline1 permet de visualiser l'effet multiplicatif de la spline spatiale sur le ratio prédit par le modèle. Ainsi, toute chose étant égale par ailleurs, le ratio de part modale TC/auto est entre 30 % et 70 % plus fort dans les quartiers centraux de Montréal, allant de Lachine à Anjou. L'extrémité Ouest de Longueuil fait aussi partie de cette zone avec le bonus spatial le plus important. Aussi, on constate que l'effet spatial diminue avec la distance au Centre-Vile. Pour la majeure partie de Laval, le ratio est réduit de prêt de 50 %.

### GAM avec spline spatiale de type RMF {#sec-0653}
...
#| warning: false
#| eval: true
#| label: tbl-GamSpatialRMK
#| tbl-cap: Résultats du modèle GLM avec spatiale de type RMK
library(knitr)
library(kableExtra)
...
Il apparaît que les résidus simulés de ce modèle suivent une distribution assez proche d'une distribution uniforme. Nous constatons aussi que toutes les variables indépendantes sont encore significatives et impactent notre variable dépendante dans la même direction (@tbl-GamSpatialRMK).
...
**Exercice 2.** Réalisation d'un modèle GAM avec une spline bivariée sur les coordonnées géographiques
:::
...
**Exercice 3.** Réalisation d'un modèle GAM avec une spline de type RMF
:::
